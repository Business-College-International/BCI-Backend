...
export class PaymentPreflightService {
  ...
  async preflight(
    studentId: string,
    dto: PaymentPreflightDto,
    actorUserId: string,
    roles: RoleName[],
  ) {
    const allowed = await this.canAccessStudentFinance(studentId, actorUserId, roles);
    if (!allowed) {
      throw new ForbiddenException('You do not have permission to prepare a payment for this student.');
    }

    const uniqueInvoiceIds = [...new Set(dto.invoiceIds)];
    if (uniqueInvoiceIds.length !== dto.invoiceIds.length) {
      throw new BadRequestException('Invoice identifiers must be unique.');
    }
    ...
    if (invoices.length !== uniqueInvoiceIds.length) {
      throw new BadRequestException('One or more selected invoices are missing, paid, void, or belong to another student.');
    }
    ...
    if (outstandingInvoices.some((item) => item.outstanding.lte(0))) {
      throw new BadRequestException('Selected invoices do not have a positive outstanding balance.');
    }

    const availableOutstanding = outstandingInvoices.reduce(
      (sum, item) => sum.plus(item.outstanding),
      new Prisma.Decimal(0),
    );
    ...
    if (requestedAmount.gt(availableOutstanding)) {
      throw new BadRequestException('Payment amount exceeds the selected invoices outstanding balance.');
    }
