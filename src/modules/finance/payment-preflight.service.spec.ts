...
describe('PaymentPreflightService', () => {
  it('rejects duplicate invoice ids', async () => {
    ...
    expect(prisma.studentInvoice.findMany).not.toHaveBeenCalled();
  });
  ...
  it('builds oldest-due-first allocation and reports pending payments', async () => {
    ...
    prisma.studentInvoice.findMany.mockResolvedValue([
      {
        id: 'invoice-1', invoiceNumber: 'BCI-1', studentId: 'student-1', termId: 'term-1',
        dueAt: new Date('2026-09-10T00:00:00Z'), issuedAt: new Date('2026-09-01T00:00:00Z'),
        lines: [{ amountDue: new Prisma.Decimal('500.00') }], allocations: [{ amount: new Prisma.Decimal('100.00') }],
        term: { id: 'term-1', code: 'T1', name: 'Term 1' },
      },
      {
        id: 'invoice-2', invoiceNumber: 'BCI-2', studentId: 'student-1', termId: 'term-1',
        dueAt: new Date('2026-10-01T00:00:00Z'), issuedAt: new Date('2026-09-02T00:00:00Z'),
        lines: [{ amountDue: new Prisma.Decimal('800.00') }], allocations: [],
        term: { id: 'term-1', code: 'T1', name: 'Term 1' },
      },
    ]);
    prisma.payment.findMany.mockResolvedValue([
      { id: 'payment-1', amount: new Prisma.Decimal('100.00'), status: 'PROCESSING', createdAt: new Date('2026-09-15T08:00:00Z'), clientReference: 'BCI-PENDING-1' },
    ]);

    const service = new PaymentPreflightService(prisma as never);
    const result = await service.preflight('student-1', { invoiceIds: ['invoice-1', 'invoice-2'], amount: '900.00' }, 'guardian-user', ['GUARDIAN'] as never);
