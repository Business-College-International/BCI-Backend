...
export class StudentRecordsService {
  ...
  async listDocuments(studentId: string, userId: string, roles: RoleName[]) {
    const student = await this.prisma.student.findUnique({ where: { id: studentId }, select: { id: true, guardians: { select: { guardian: { select: { userId: true } }, canViewAcademic: true } }, enrolments: { where: { status: 'ACTIVE' }, orderBy: { enrolledAt: 'desc' }, take: 1, select: { classId: true, termId: true } } } });
    ...
    if (roles.includes(RoleName.TEACHER) && student.enrolments[0]) {
      ...
      if (staff) {
        const assignment = await this.prisma.teacherAssignment.findFirst({ where: { staffId: staff.personId, classId: student.enrolments[0].classId, termId: student.enrolments[0].termId }, select: { id: true } });
    ...
    return this.prisma.studentDocument.findMany({ where: { studentId }, orderBy: { createdAt: 'desc' }, select: { id: true, type: true, fileUrl: true, createdAt: true } });
  }
