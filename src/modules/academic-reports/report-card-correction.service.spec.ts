...
describe('ReportCardCorrectionService', () => {
  ...
  it('rejects approval when the authoritative snapshot has no effective change', async () => {
    const { prisma } = makePrisma();
    const baselineSnapshot = snapshotFor(report);
    const request = {
      id: 'corr-noop',
    ...
    await expect(service.approve('corr-noop', 'Reviewed and found no effective change.', 'principal-1', [RoleName.PRINCIPAL]))
      .rejects.toBeInstanceOf(ConflictException);
  });
