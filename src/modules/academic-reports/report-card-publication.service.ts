...
export class ReportCardPublicationService {
  ...
  async publish(id: string, actorUserId: string, roles: RoleName[]) {
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (existing) throw new ConflictException('Another published report exists for this student and term.');
        ...
        await tx.auditLog.create({
          data: {
            ...
            entityType: 'ReportCardPublication',
            entityId: id,
            beforeJson: { status: publication.status },
  ...
  async void(id: string, actorUserId: string, roles: RoleName[], reason: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const publication = await tx.reportCardPublication.findUnique({ where: { id } });
      if (!publication) throw new NotFoundException('Report-card publication not found.');
      assertPublicationTransition(publication.status as any, ReportCardPublicationStatus.VOIDED);
  ...
  async current(studentId: string, termId: string, actorUserId: string, roles: RoleName[]) {
    const scope = await this.reports.getStudentTermSummary(studentId, termId, actorUserId, roles);
    if (!scope) throw new NotFoundException('Report not available.');
  ...
  private assertPublishRole(roles: RoleName[]) {
    if (!roles.some((role) => PUBLISH_ROLES.has(role))) {
      throw new ForbiddenException('Only authorized academic/office leadership may publish report cards.');
    }
  }
}
