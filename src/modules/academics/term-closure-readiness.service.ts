...
export class TermClosureReadinessService {
  ...
  async assertReadyForClosure(tx: Prisma.TransactionClient, termId: string) {
    ...
    if (result.blockers.length > 0) {
      throw new BadRequestException(
        'Term is not ready for closure: ' + result.blockers.join(', '),
      );
    }
    return result;
  }
  ...
  private async evaluate(
    ...
    if (term.status !== 'OPEN') {
      throw new BadRequestException(
        'Closure readiness is only available for an open term.',
      );
    }
    ...
    const [
      ...
      client.assessmentResult.findMany({
      ...
      client.attendanceSession.count({
        where: { termId, publishedAt: { not: null } },
      }),
    ]);

    const activeStudentIds = new Set(
      enrolments.map((entry) => entry.studentId),
    );
    ...
    for (const result of assessmentResults) {
      ...
      const existing =
        resultsByStudent.get(result.studentId) ?? new Set<string>();
      existing.add(result.assessmentId);
      resultsByStudent.set(result.studentId, existing);
    }

    const studentsWithMissingAssessmentResults =
      assessments.length === 0
        ? activeStudentIds.size
        : Array.from(activeStudentIds).filter(
            (studentId) =>
              (resultsByStudent.get(studentId)?.size ?? 0) <
              assessments.length,
          ).length;
    ...
    return {
      ...
      blockers: [
        ...
        ...(assessments.length === 0 && enrolments.length > 0
          ? ['NO_ASSESSMENTS']
          : []),
        ...(studentsWithMissingAssessmentResults > 0
          ? ['MISSING_ASSESSMENT_RESULTS']
          : []),
        ...(attendanceSessions > publishedAttendanceSessions
          ? ['UNPUBLISHED_ATTENDANCE_SESSIONS']
          : []),
      ],
    };
  }
